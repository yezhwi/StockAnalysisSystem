...
public class FiveGItemShow extends AppCompatActivity implements AdapterView.OnItemClickListener{
    private ListView fg_item_show_listView;
    private TextView page_name,stock_num;
    private List<fgStock> fgStockList;
    private FiveGStockAdapter adapter;
    public static Stock stock=new Stock();
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_five_gitem_show);
        fg_item_show_listView =(ListView)findViewById(R.id.fg_item_show_listView);
       ...
       //Toast.makeText(getBaseContext(),categoryname+","+categoryid,Toast.LENGTH_SHORT).show();
        ...
        fgStockList= LitePal.select()
                .where("category=?",String.valueOf(categoryid))
                .find(fgStock.class);
        stock_num.setText(String.valueOf(fgStockList.size()));
        adapter=new FiveGStockAdapter(getBaseContext(),fgStockList,categoryname);
        fg_item_show_listView.setAdapter(adapter);
        fg_item_show_listView.setOnItemClickListener(this);
    }
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        fgStock fgStock = (fgStock)parent.getItemAtPosition(position);
        search(fgStock.getGid());
