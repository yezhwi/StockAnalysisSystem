...
public class charge_bar_show extends AppCompatActivity implements AdapterView.OnItemClickListener{
    private ListView green_car_listView;
    private TextView page_name,stock_num;
    private List<greenCarStock> greenCarStockList;
    private green_car_show_adapter adapter;
    public static Stock stock=new Stock();
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_charge_bar_show);
        green_car_listView=(ListView)findViewById(R.id.green_car_listView);
        ...
        greenCarStockList= LitePal.select()
                .where("category=?",categoryid)
                .find(greenCarStock.class);
        stock_num.setText(String.valueOf(greenCarStockList.size()));
        adapter=new green_car_show_adapter(getBaseContext(),greenCarStockList,categoryname);
        green_car_listView.setAdapter(adapter);
        green_car_listView.setOnItemClickListener(this);
    }
    @Override
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        greenCarStock greenCarStock = (greenCarStock)parent.getItemAtPosition(position);
        search(greenCarStock.getGid());
