...
public class lowPriceStock extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    private Handler lowPricehandler =new Handler(){
        public void handleMessage(Message msg){
            ...
            if(allStock!=null&&allStock.size()>0){
                LowPriceAdapter listAdapter=new LowPriceAdapter(getBaseContext(),allStock);
...
//                        .order("publishdate desc").limit(10).find(News.class);
                ...
                List<StockInfoDB> allStocks= LitePal.select()
                        .where("categoryID!=? and categoryID!=? and now_Price>? and volume>? ","4","3","0.2","500")
                        .order("now_Price asc")
                        .limit(100)
