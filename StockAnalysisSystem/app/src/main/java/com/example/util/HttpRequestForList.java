...
public class HttpRequestForList {
    ...
    public static void parseJSONWithJSONObject(Context context, String jsonData) {
        ...
        try {
            ...
            JSONObject jsonArray1 = root_json.getJSONObject("result");//获取root_json中的result作为jsonArray对象
            ...
            for (int i = 0 ;i < jsonArray.length();i++){//循环遍历jsonArray
                JSONObject stocks_json = jsonArray.getJSONObject(i);//获取一条股票的json
                Stock stock = new Stock();
                stock.trade=stocks_json.getString("trade");
                stock.symbol=stocks_json.getString("symbol");
                stock.name=stocks_json.getString("name");
                stock.pricechange=stocks_json.getString("pricechange");
                stock.changepercent=stocks_json.getString("changepercent");
                stock.open=stocks_json.getString("open");
                stock.high=stocks_json.getString("high");
                stock.low=stocks_json.getString("low");
                stock.close=stocks_json.getString("settlement");
                arrayList.add(stock);
    ...
    public static void parseJSONWithJSONObject1(Context context, String jsonData) {
        ...
        try {
            ...
            String data=stocks_json.toString();
                ...
                mystock.trade=stocks_json.getString("nowPri");
                mystock.pricechange=stocks_json.getString("increase");
                mystock.changepercent=stocks_json.getString("increPer");
                mystock.date=stocks_json.getString("date");
                mystock.time=stocks_json.getString("time");
                mystock.open=stocks_json.getString("todayStartPri");
                mystock.high=stocks_json.getString("todayMax");
                mystock.low=stocks_json.getString("todayMin");
                mystock.close=stocks_json.getString("yestodEndPri");
            } catch (JSONException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }//将一个字符串封装成一个json对象。
    ...
    public static void  UpdateParseJSONWithJSONObject(Context context, String jsonData) {
        ...
        try {
            root_json = new JSONObject(jsonData);
            JSONObject jsonArray2 = root_json.getJSONObject("result");//获取root_json中的result作为jsonArray对象
            ...
            for (int i = 0 ;i < jsonArray.length();i++){//循环遍历jsonArray
                ...
                *    now_Price;//trade，港股里的lasttrade，美股里的price
                ...
                stockInfoDB.now_Price=Double.parseDouble(stocks_json.getString("trade"));
                stockInfoDB.name=stocks_json.getString("name");
                stockInfoDB.price_change=Double.parseDouble(stocks_json.getString("pricechange"));
                stockInfoDB.change_percent=Double.parseDouble(stocks_json.getString("changepercent"));
